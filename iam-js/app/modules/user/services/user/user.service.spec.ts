...
describe('UserService', () => {
    ...
    beforeEach(async () => {
        ...
        userRepository = { create: stub(), save: stub(), find: stub(), findOne: stub(), remove: stub() };
    ...
    describe('findAll', () => {
        it('should return all users with their roles and permissions', async () => {
            ...
            expect(userRepository.find.calledOnceWith({ relations: ['roles', 'roles.permissions'] })).toBeTruthy();
    ...
    describe('findOne', () => {
        it('should return the user matching the given id', async () => {
            ...
            expect(userRepository.findOne.calledOnceWith({ where: { id: USER_ID }, relations: ['roles', 'roles.permissions'] })).toBeTruthy();
            expect(result).toEqual(USERS[0]);
        });
    ...
    describe('findByEmail', () => {
        it('should return the user matching the given email', async () => {
            ...
            expect(userRepository.findOne.calledOnceWith({ where: { email: USER_EMAIL }, relations: ['roles', 'roles.permissions'] })).toBeTruthy();
            expect(result).toEqual(USERS[0]);
        });
    ...
    describe('update', () => {
        ...
        it('should throw ConflictException if the new email is already taken', async () => {
            userRepository.findOne
                .onFirstCall().resolves(USERS[0])   // findOne (by id)
                .onSecondCall().resolves(USERS[1]);  // findByEmail

            await expect(service.update(USER_ID, { email: 'jane.doe@example.com' })).rejects.toThrow(ConflictException);
        });

        it('should throw NotFoundException if user does not exist', async () => {
            userRepository.findOne.resolves(null);

            await expect(service.update(USER_ID, { firstName: 'Jack' })).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('assignRole', () => {
        it('should assign a new role to the user and save', async () => {
            ...
            userRepository.save.resolves({ ...user, roles: [...user.roles, newRole] });
        ...
        it('should throw NotFoundException if user does not exist', async () => {
            ...
            await expect(service.assignRole(USER_ID, ROLES[0] as unknown as Role)).rejects.toThrow(NotFoundException);
        });
    });
});