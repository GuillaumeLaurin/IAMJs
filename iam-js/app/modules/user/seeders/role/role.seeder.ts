...
export class RoleSeeder implements OnApplicationBootstrap {
    ...
    async onApplicationBootstrap(): Promise<void> {
        ...
        const rolesToDelete: SeedingInterface[] = Array.from(mappedRoles.values()).filter((val) => val.keepIt === false);

        const rolesToCreate: SeedingInterface[] = Array.from(mappedRoles.values()).filter((val) => val.isNew === true);

        const rolesToUpdate: SeedingInterface[] = Array.from(mappedRoles.values()).filter((val) => val.needsUpdate === true);

        const seedingReport: SeedingReport = { deletedRoles: 0, updatedRoles: 0, createdRoles: 0, skippedRoles: 0, deleteResults: [] };

        seedingReport.skippedRoles = permissions.length ? permissions.length - (rolesToDelete.length + rolesToUpdate.length) : 0;
        ...
        for (const role of rolesToDelete) {
            ...
            deleteResult.affected = (await this.rolesRepository.delete({ id: role.id }))?.affected;
        ...
        for (const role of rolesToCreate) {
            const perms = role.permissions.map((val) => savedPermissions.get(val));
            await this.rolesRepository.create({ name: role.name, permissions: perms });
